...
class ShowDynamicFormDataAdapter(var formDataList: ArrayList<TaskData>) : RecyclerView.Adapter<BaseSdkViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BaseSdkViewHolder {
        ...
        when (viewType) {
            VIEW_DISPLAY_TEXT -> {
                val itemFormPreview = ItemLayoutFormDetailsSdkBinding
                        .inflate(LayoutInflater.from(parent.context), parent, false)
                return FormPreviewViewHolder(itemFormPreview)
            }
            VIEW_DISPLAY_SINGLE_PIC -> {
                val itemFormPicture = ItemLayoutSingleImagesSdkBinding
                        .inflate(LayoutInflater.from(parent.context), parent, false)
                return FormSinglePictureViewHolder(itemFormPicture)
            }
            VIEW_DISPLAY_PICS -> {
                val itemFormPicture = ItemRowImageFormDetailsSdkBinding
                        .inflate(LayoutInflater.from(parent.context), parent, false)
                return FormPictureViewHolder(itemFormPicture)
            }
            VIEW_PLAY_AUDIO -> {
                val itemFormPicture = ItemLayoutPlayAudioDetailsSdkBinding
                        .inflate(LayoutInflater.from(parent.context), parent, false)
                return FormAudioViewHolder(itemFormPicture)
            }
            VIEW_PLAY_VIDEO -> {
                val itemFormPicture = ItemLayoutPreviewVideoSdkBinding
                        .inflate(LayoutInflater.from(parent.context), parent, false)
                return FormVideoViewHolder(itemFormPicture)
            }

            else -> {
                val emptyViewBinding = ItemDynamicFormEmptyViewSdkBinding
                        .inflate(LayoutInflater.from(parent.context), parent, false)
