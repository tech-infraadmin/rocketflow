...
import com.rf.taskmodule.data.model.response.config.Location
...
class TripDetailsViewModel(dataManager: DataManager?, schedulerProvider: SchedulerProvider?) : BaseSdkViewModel<TripDetailsNavigator>(
    dataManager, schedulerProvider
) , ApiCallback, LocationObserver {
...
//            }
    ...
    private fun createListOfPolygonAndMarkers(eventList: ArrayList<Event>,
        ...
        if (eventList.size == 1) {
            markersList.add(
                MarkerOptions()
                    .position(
                        LatLng(eventList[0].from?.latitude!!,
    ...
    internal class Factory(private val mDataManager: DataManager) : ViewModelProvider.Factory {
        override fun <T : ViewModel> create(modelClass: Class<T>): T {
            return TripDetailsViewModel(mDataManager,
