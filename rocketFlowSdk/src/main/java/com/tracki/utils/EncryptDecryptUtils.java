...
/**
 * Created by Vikas  on 20/10/2020.
 */
...
public class EncryptDecryptUtils {
    ...
    public static byte[] encode(SecretKey yourKey, byte[] fileData)
        ...
        SecretKeySpec skeySpec = new SecretKeySpec(data, 0, data.length, AppConstants.KEY_SPEC_ALGORITHM);
        Cipher cipher = Cipher.getInstance(AppConstants.CIPHER_ALGORITHM, AppConstants.PROVIDER);
    ...
    public SecretKey getSecretKey() {
        ...
        if (null == encodedKey || encodedKey.isEmpty()) {
            ...
            try {
                keyGenerator = KeyGenerator.getInstance(AppConstants.KEY_SPEC_ALGORITHM);
            } catch (NoSuchAlgorithmException e) {
                e.printStackTrace();
            }
            keyGenerator.init(AppConstants.OUTPUT_KEY_LENGTH, secureRandom);
        ...
        SecretKey originalKey = new SecretKeySpec(decodedKey, 0, decodedKey.length, AppConstants.KEY_SPEC_ALGORITHM);
        return originalKey;
    }
