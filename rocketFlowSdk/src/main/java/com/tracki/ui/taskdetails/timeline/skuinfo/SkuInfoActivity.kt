...
import com.tracki.databinding.ActivitySkuInfoBinding
import com.tracki.databinding.ItemDynamicFormVideoBinding
import com.tracki.ui.addplace.HubLocation
...
open class SkuInfoActivity : BaseActivity<ActivitySkuInfoBinding, SkuInfoViewModel>(), SkuInfoNavigator,
    ...
    lateinit var binding: ActivitySkuInfoBinding
    ...
    override fun getLayoutId(): Int {
        return R.layout.activity_sku_info
    }
    ...
    private fun finalApiHit(mainData: ArrayList<FormData>) {
        ...
        val jsonConverterDataToUpload = JSONConverter<HashMap<String,ArrayList<FormData>>>()
       ...
       val data = ArrayList<ProductItem>()
        ...
        val skuSpecsJson = JSONConverter<SKUInfoSpecsRequest>()
        val skuSpecsData = skuSpecsJson.objectToJson(skuSpecs)
        Log.e("Final Data skuSpecsData--->", skuSpecsData)
        if (NetworkUtils.isNetworkConnected(this)) {
            showLoading()
            val api = TrackiApplication.getApiMap()[ApiType.SAVE_TASK_UNITS]
...
//        startActivityForResult(scanActivity, AppConstants.REQUEST_CODE_SCAN)
    ...
    fun setVid(pos: Int, file: File, path: String) {
        showDynamicFormDataAdapter.itemDynamicFormVideoBinding = mBinding
    ...
    fun setChildVideo(pos: Int, file: File, path: String) {
        if (showDynamicFormDataAdapter.childDynamicAdapter != null) {
            showDynamicFormDataAdapter.childDynamicAdapter.itemDynamicFormVideoBinding = mBinding
        ...
        override fun handleMessage(message: Message) {
            when (message.what) {
                ...
                if (count == 0) {
                    ...
                    count++
                    fileUploadCounter = 0
                    TrackiToast.Message.showShort(this@SkuInfoActivity,AppConstants.UNABLE_TO_PROCESS_REQUEST)
                    handlerThread?.interrupt()
                    hideLoading()
                }
            }
